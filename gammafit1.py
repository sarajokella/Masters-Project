...
def patchROOTsVectorAndMatrix(typ):
    """ helper to patch some of the botched pythonization of ROOT matrix/vector classes """
    def add(typ, m1, m2):
        retVal = typ(m1)
        retVal += m2
        return retVal
...
def buildFullCovMatrix(errStat, correlStat, errSyst, correlSyst):
    """ build a full covariance matrix based on stat. and syst. errors and their correlations """
...
def gauss(x, mu, icov, det):
    """
    return the value of a multiunivariate Gaussian with mean mu,
    inverse covariance matrix icov, and determinant of the
    covariance matrix det
    """
...
def plotContours(npar1, npar2, title, name1, name2, sigmas = [ 1., 2. ], colors =
        [ ROOT.kOrange, ROOT.kGreen, ROOT.kBlue ], npoints = 150):
    first = True
